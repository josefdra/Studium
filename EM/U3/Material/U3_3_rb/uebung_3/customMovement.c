...
void drive_forward(int distanceInCm, bool direction){
    Motor_Drive(ENG_L, direction, 75*ENG_L_FINEADJ);
    Motor_Drive(ENG_R, direction, 75*ENG_R_FINEADJ);
...
void turn(bool direction, int degrees){
     Motor_Drive(ENG_L, FORWARD^direction, 75*ENG_L_FINEADJ);
     Motor_Drive(ENG_R, BACKWARD^direction, 75*ENG_R_FINEADJ);
     Delay(20*degrees);
     Motor_Stop(ENG_L, Motor_stop_float);
     Motor_Stop(ENG_R, Motor_stop_float);
}
